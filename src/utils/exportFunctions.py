...
def export2sta(P,L,M,O,N,filename):
    start = time.time()
    f = open(filename,'w')
    ...
    for i,((c,e),o) in enumerate(P.S):
        ...
        line = str(i)+':('+ \
            ...
            str(L.names[c][1])+','
        # TOFIX
        if type(M.names[e][0]) is tuple: 
            for i in range(len(M.names[e])):
                line += str(M.names[e][i][0]) + ',' + str(M.names[e][i][1]) + ','
        elif type(M.names[e][0]) is int:
            line += str(M.names[e][0]) + ',' + str(M.names[e][1]) + ','
        line += str(inv_obs[o]) + ')\n'
        f.write(line)
